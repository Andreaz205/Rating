...
const Product:FC<{data: ProductType}> = ({data}) => {
    ...
    if (data.reviews.rating > 5) data.reviews.rating = 5

    let numberFilledStars = useMemo(() => {
        return Math.trunc(data.reviews.rating)
    }, [data.reviews.rating])

    let remainder = useMemo(() => {
        return Number((data.reviews.rating - numberFilledStars + 0.01).toFixed(1))
    }, [data.reviews.rating, numberFilledStars])
    console.log('render')
